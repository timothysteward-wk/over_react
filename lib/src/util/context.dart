...
/// [ConsumerProps] is a typed props class for the [Context.Consumer]
/// from a [Context] object created with [createContext].
///
...
Context<TValue> createContext<TValue>([TValue defaultValue, int Function(TValue, TValue) calculateChangedBits]) {
  ...
  UiFactory<ProviderProps> Provider = ([map]) => (ProviderProps<TValue>(map)..componentFactory = reactDartContext.Provider);
  // ignore: prefer_function_declarations_over_variables
  UiFactory<ConsumerProps> Consumer = ([map]) => (ConsumerProps<TValue>(map)..componentFactory = reactDartContext.Consumer);
